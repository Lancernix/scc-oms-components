...
export default function Index() {
  ...
  return (
    <>
      <Form form={form}>
        <Form.Item name="create_time" initialValue={[dayjs(), dayjs().add(30, 'minutes')]}>
      ...
      <div style={{ marginTop: '20px' }}>
        <span>
          当前时区的值为：
          {JSON.stringify({
            create_time_start: form.getFieldValue('create_time')?.[0]?.format('HH:mm:ss'),
            create_time_end: form.getFieldValue('create_time')?.[0]?.format('HH:mm:ss'),
          })}
        </span>
        <br />
        <span>
